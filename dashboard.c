...
{
    composedWindow *composedwindow = (composedWindow *)data;

    //? dashboard window
    MainWindow *App = composedwindow->App;
    gtk_widget_hide(App->new_user_window);
    ...
    gtk_label_set_markup(GTK_LABEL(App->dashboard_selected_user_label), "USERNAME");
...
void handleBackDashboard(gpointer data)
{
    MainWindow *App;
    App = (MainWindow *)data;
    // gtk_widget_show(App->new_user_window);
    // gtk_widget_hide(App->dashboard_window);
}